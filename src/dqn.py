...
def learn(env,
    ...
    target_q_values = rew_t_ph + gamma*tf.reduce_max(next_q_values, axis=1)
    indexed_q_values = tf.one_hot(act_t_ph, depth=num_actions, dtype=tf.float32)
    actual_q_values = tf.reduce_sum(indexed_q_values*q_values, axis=1)
    total_error = tf.reduce_mean(tf.square(tf.subtract(target_q_values, actual_q_values)))
    ...
    for t in itertools.count():
        ...
        if t % SAVE_EVERY_N_STEPS == 0 and model_initialized:
            log = ({'ts': ts, 'mean_rewards': mean_rewards, 'best_mean_rewards':best_mean_rewards, 'episodes': episodes,
                'explorations': exploration, 'learning_rates': learning_rates})
